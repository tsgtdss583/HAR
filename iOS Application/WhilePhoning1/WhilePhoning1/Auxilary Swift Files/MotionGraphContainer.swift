...
extension MotionGraphContainer {
    ...
    var formattedUpdateInterval: String {
        updateIntervalFormatter.numberFormatter.minimumFractionDigits = 3 // min #of digits after decimal, u can change it
        updateIntervalFormatter.numberFormatter.maximumFractionDigits = 3
        
        let updateInterval = Measurement(value: Double(updateIntervalSlider.value), unit: UnitDuration.seconds) //consider changing it by let up = Measurement(value: 1/b, unit: UnitFrequency.hertz)
